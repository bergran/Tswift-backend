...
class TasksTestRetrieve(APITestCase):
    def setUp(self):
        # Create some users
    ...
    def launch_successfull_test(self, user, task):
        ...
        self.check_data_fields(task, payload)
    ...
    def check_data_fields(self, task, payload):
        self.assertEqual(task.id, payload.get('id'))
        self.assertEqual(task.title, payload.get('title'))
        self.assertEqual(task.description, payload.get('description'))
        self.assertEqual(
            task.date_created,
