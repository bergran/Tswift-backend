...
class DeleteGroupsTestBoard(APITestCase):
    def setUp(self):
        # Create some users
        ...
        self.group_permission_1 = GroupBoardPermissions.objects.create(
            group=self.group2,
            board=self.board1,
            permission=self.read
        )

        self.group_permission_2 = GroupBoardPermissions.objects.create(
            group=self.group2,
            board=self.board1,
            permission=self.write
        )

        self.group_permission_5 = GroupBoardPermissions.objects.create(
            group=self.group2,
            board=self.board1,
            permission=self.delete
        )

        self.group_permission_3 = GroupBoardPermissions.objects.create(
            group=self.group3,
            board=self.board1,
            permission=self.read
        )

        self.group_permission_4 = GroupBoardPermissions.objects.create(
            group=self.group3,
            board=self.board1,
            permission=self.write
        )

        self.group_permission_6 = GroupBoardPermissions.objects.create(
            group=self.group3,
            board=self.board1,
            permission=self.delete
        )
    ...
    def get_uri(self, pk):
        return '/api/v1/boards/{}/delete_groups/'.format(pk)
    ...
    def launch_test_successfully(self, user, board, params):
        ...
        self.assertFalse(GroupBoardPermissions.objects.filter(
            pk__in=[
                self.group_permission_1.pk,
                self.group_permission_2.pk,
                self.group_permission_3.pk,
                self.group_permission_4.pk
            ]
        ).exists())

        self.assertTrue(GroupBoardPermissions.objects.filter(
            pk__in=[
                self.group_permission_5.pk,
                self.group_permission_6.pk
            ]
        ).exists())
    ...
    def test_add_groups_successfully(self):
        ...
        board = self.board1
        params = {
            'groups': [self.group2.pk, self.group3.pk],
            'permissions': [self.write.name, self.read.name]
        }
