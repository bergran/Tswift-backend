...
class StatesTestRetrieve(APITestCase):
    ...
    def send_request_with_authenticate(self, user, pk, data):
        # Get uri
        uri = self.get_uri(pk)
    ...
    def launch_test_successfully(self, user, state, data):
        # Send request
        ...
        self.assertNotEqual(parse_datetime(
            payload.get('date_modified')),
            state.date_modified
        )
        self.assertEqual(payload.get('deleted'), state.deleted)
        self.assertEqual(payload.get('board'), data.get('board'))

    def test_update_status_state_1_user1(self):
        # Params
        user = self.user1
        state = self.state1
        data = {
        ...
        self.launch_test_successfully(user, state, data)

    def test_update_status_state_3_user2(self):
        # Params
        user = self.user2
        state = self.state3
        data = {
    ...
    def test_update_status_state_3_user1(self):
        # Params
        user = self.user1
        state = self.state3
        data = {
        ...
        # Send request
        response = self.send_request_with_authenticate(user, state.pk, data)

        # Check response status code is equals to 403
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_update_status_state_1_user2_forbidden(self):
        # Params
        user = self.user2
        state = self.state1
        data = {
